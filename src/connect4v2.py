...
class Connect4():
    def __init__(self):
        ...
        self.header_text = self.canvas.create_text(self.width/2, 50, 
            text = ("It is Player 1's turn! (" + player_colors[self.current_num] + ")"))
    ...
    def draw_piece(self, move):
        ...
        self.canvas.create_oval((move - 1) * 100 + 25, 
                                ...
                                move * 100 + 25, self.height + 75 - (100 * col_length), 
                                fill = player_colors[self.current_num])
        self.canvas.after(100, self.canvas.update)
    
    def play_and_draw_move(self, move, board = None, current_num = None):
        if not board:
            board = self.board
        if not current_num:
    ...
    def run_game(self):
        ...
        if self.ai_first:
            self.player1, self.player2 = AI_Player(1, self), Human_Player(2, self)
        else:
            self.player1, self.player2 = Human_Player(1, self), AI_Player(2, self)
        self.current_player = self.player1
